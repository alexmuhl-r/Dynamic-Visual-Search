...
#	- Our preprint on PsyArXiv: Muhl-Richardson, A., Godwin, H., Garner, M., Dr, Hadwin, J., Liversedge, S. P., & Donnelly, N. (2019). Examining the Impact of Working Memory Training and Transcranial Direct-current Stimulation on Monitoring for Color Targets in Dynamic Visual Displays. https://doi.org/10.31234/osf.io/kyxb8
...
class CustomClassTemplate(sreb.EBObject):
	...
	def initialize(self):
		...
		if int(self.ppt_id) > 63:
			self.rotation = int(self.ppt_id) - 64
			
		if int(self.ppt_id) > 79:
			self.rotation = int(self.ppt_id) - 80
			
		if int(self.ppt_id) > 95:
			self.rotation = int(self.ppt_id) - 96
	...
	def trialStructure(self):
		...
		#50% of trials will have 5 targets
		if int(self.prevalence) == 1:
			for i in range (0, 36):
				self.trialArray.append(0)
			for i in range (0, 36):
				self.trialArray.append(5)
	...
	def reset(self):
		...
		if self.targetsInTrial == 5:
			newObjectTypes = ["TARGET", "TARGET", "TARGET", "TARGET", "TARGET", "DISTRACTOR", "DISTRACTOR", "DISTRACTOR", "DISTRACTOR", "DISTRACTOR", "DISTRACTOR", "DISTRACTOR"]
